...
- (void)viewDidLoad {
    ...
    testFile= [NSData dataWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"SQL语句教程" ofType:@"pdf"]];
    
    UIColor *blue=[UIColor colorFromHex:@"#4799dd"];
    ...
    int i=0;int buttonWidth=APPW/icons.count-20;
    if(buttonWidth>80){
        buttonWidth=80;
    }
    float fromX=APPW/2-icons.count*(buttonWidth+10)/2;
    if (fromX<0) {
        fromX=0;
    }
    for (NSString *icon in @[@"weibo",@"qq",@"weixin",@"renren",@"alipay"]/*对dictionary进行for-in，不能保证顺序*/) {
        ...
        btn.frame=CGRectMake(APPW/2-buttonWidth/2, TopHeight+10, buttonWidth, buttonWidth);
        btn.layer.borderColor=blue.CGColor;
        btn.layer.borderWidth=1;
        [btn setBackgroundImage:[UIImage imageWithColor:[UIColor whiteColor] inRect:CGRectMake(0.0f, 0.0f, 1.0f, 1.0f)] forState:UIControlStateNormal];
        [btn setBackgroundImage:[UIImage imageWithColor:blue inRect:CGRectMake(0.0f, 0.0f, 1.0f, 1.0f)] forState:UIControlStateSelected];
    ...
    float fromY=YH([self.view viewWithTag:1])+10;
    panel=[[UIScrollView alloc] initWithFrame:CGRectMake(0,fromY , APPW, APPH-fromY)];
    [self.view addSubview:panel];
    panel.hidden=YES;
    panel.contentSize=CGSizeMake(APPW*(icons.count+1), APPH-fromY);
    ...
    //测试分享的view
    CGRect frame=CGRectMake(0, 10, APPW-fromX*2, panel.frame.size.height);
    ...
    for (int i=1; i<=icons.count; i++) {
        UIView *view=views[i];
        view.tag=100+i;
        view.frame=CGRectMake(i*APPW+fromX, view.frame.origin.y, view.frame.size.width,view.frame.size.height);
    ...
    [UIView animateWithDuration:2 delay:0.5 usingSpringWithDamping:YES initialSpringVelocity:20 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        for (int i=1; i<=icons.count; i++) {
            [self.view viewWithTag:i].frame=CGRectMake(fromX+(i-1)*(buttonWidth+10), TopHeight+10, buttonWidth, buttonWidth);
        }
...
-(UIView*)alipayView:(CGRect)frame{
    ...
    [alipay addEventHandler:^(UIButton* sender) {
        ...
        if ([apiUrl hasPrefix:@"https://pay.example.com"]) {
            DLog(@"请部署pay.php，填写自家的key。否则无法测试。");
        }else{
            //网络请求不要阻塞UI，仅限Demo
...
-(UIView*)sinaWeiboView:(CGRect)frame{
    ...
    UIButton *textShare=[self button:@"分享纯文本" WithCenter:CGPointMake(auth.center.x, YH(auth)+40)];
    ...
    UIButton *imgShare=[self button:@"分享图片" WithCenter:CGPointMake(auth.center.x, YH(textShare)+40)];
    ...
    UIButton *newsShare=[self button:@"分享新闻" WithCenter:CGPointMake(auth.center.x, YH(imgShare)+40)];
...
-(UIView*)qqView:(CGRect)frame{
    ...
    UIButton *chat=[self button:@"和我聊天" WithCenter:CGPointMake(frame.size.width/2, YH(auth)+40)];
    [loginView addSubview:chat];
    [chat addEventHandler:^(id sender) {
        [OpenShare chatWithQQNumber:@"393475141"];
    } forControlEvents:UIControlEventTouchUpInside];
    
    UIButton *chatGroup=[self button:@"指定群聊天(必须是群成员)" WithCenter:CGPointMake(frame.size.width/2, YH(chat)+40)];
...
-(UIView*)weixinView:(CGRect)frame{
    ...
    UIView *loginView=[[UIView alloc]initWithFrame:CGRectMake(0, YH(seg)+30, frame.size.width, frame.size.height)];
    loginView.backgroundColor=[UIColor whiteColor];
...
-(void)weixinViewHandler:(UIButton*)btn{
    ...
    if (btn.tag==30002) {
        ...
        msg.file=testFile;
        msg.thumbnail=testThumbImage;
        msg.title=@"SQL语句教程.pdf";//添加到收藏的时候，微信会根据文件名打开。fileExt信息丢失。微信的bug
        msg.fileExt=@"pdf";
...
-(UIView*)renrenView:(CGRect)frame{
    ...
    for (int i=0; i<titles.count; i++) {
        UIButton *btn=[self button:titles[i] WithCenter:CGPointMake(frame.size.width/2,YH(seg)+ 40+i*40)];
        [ret addSubview:btn];
        btn.tag=40001+i;
...
-(void)btnClicked:(UIButton *)btn{
    ...
    [panel setContentOffset:CGPointMake(btn.selected? btn.tag*APPW:0, 0) animated:YES];
}
@end
