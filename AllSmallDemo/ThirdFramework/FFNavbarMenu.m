...
- (instancetype)initWithItems:(NSArray *)items width:(CGFloat)width maximumNumberInRow:(NSInteger)max {
    CGFloat parentX = [UIScreen mainScreen].bounds.size.width - width;
    self = [super initWithFrame:CGRectMake(parentX, 0, width, 0)];
    if (self == nil) return nil;
    _items = items;_open = NO;
    _maximumNumberInRow = max;
    _numberOfRow = (_items.count-1)/_maximumNumberInRow + 1;
    self.frameHeight = (_numberOfRow+1) * rowHeight;
...
- (void)layoutSubviews {
    ...
    CGFloat buttonWidth = self.frameWidth/self.maximumNumberInRow;
...
- (void)showInNavigationController:(UINavigationController *)nvc {
    ...
    if (CGRectEqualToRect(self.beforeAnimationFrame, self.afterAnimationFrame)) {
        ...
        tmp.origin.y += ([UIApplication sharedApplication].statusBarFrame.size.height+nvc.navigationBar.frameHeight+rowHeight*self.numberOfRow);
        self.afterAnimationFrame = tmp;
    }
    self.background.frame = nvc.view.frame;
    [UIView animateWithDuration:0.5 delay:0.0 usingSpringWithDamping:0.6 initialSpringVelocity:1.0 options:UIViewAnimationOptionBeginFromCurrentState | UIViewAnimationOptionCurveEaseInOut animations:^{
            self.frameY = self.afterAnimationFrame.origin.y;
    } completion:^(BOOL finished) {
         if (self.delegate != nil) {[self.delegate didShowMenu:self];}
         self.open = YES;
    }];
}
- (void)dismissWithAnimation:(BOOL)animation {
    void (^completion)(void) = ^void(void) {
        ...
        if (self.delegate != nil) {[self.delegate didDismissMenu:self];}
        self.open = NO;
    };
    if (animation) {
        [UIView animateWithDuration:0.2 animations:^{
            ...
            [UIView animateWithDuration:0.1 animations:^{
                self.frameY = self.beforeAnimationFrame.origin.y;
