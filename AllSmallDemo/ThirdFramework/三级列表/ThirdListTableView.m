...
@end
@implementation ThirdListTableView
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    ...
    return [self dataSource_numberOfRowsInSection:section];
}
...
#pragma mark - Private
-(void)selectDetailOpenCell{
    if(self.delegate && [self.delegate respondsToSelector:@selector(mTableView:didSelectOpenCellAtIndexPath:)]){
        [self.delegate mTableView:self didSelectOpenCellAtIndexPath:self.selectIndexPath];
    }
}
- (void)insertViewForExpandWithIndexPath:(NSIndexPath *)indexPath{
    ...
    [self.viewForExpand addGestureRecognizer:[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(selectDetailOpenCell)]];
    ...
    __block __unsafe_unretained ThirdListTableView *weekSelf = self;
    [UIView transitionWithView:cell duration:0.3 options:UIViewAnimationOptionCurveEaseOut animations:^{
...
- (void)removeViewForExpandWithIndexPath:(NSIndexPath *)indexPath{
    if (self.viewForExpand){
        ...
        [UIView transitionWithView:self.viewForExpand duration:0.2 options:UIViewAnimationOptionCurveEaseIn animations:^{
...
- (NSInteger)dataSource_numberOfSectionsInTableView{
    NSInteger n = 1;
    if (self.delegate && [self.delegate respondsToSelector:@selector(numberOfSectionsInTableView:)]){
        n = [self.delegate numberOfSectionsInTableView:self];
    }return n;
}
- (NSInteger)dataSource_numberOfRowsInSection:(NSInteger)section{
    NSInteger n = 0;
    if (self.delegate && [self.delegate respondsToSelector:@selector(mTableView: numberOfRowsInSection:)]){
        n = [self.delegate mTableView:self numberOfRowsInSection:section];
    }return n;
}
- (UITableViewCell *)dataSource_cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    UITableViewCell *cell = nil;
    if (self.delegate && [self.delegate respondsToSelector:@selector(mTableView: cellForRowAtIndexPath:)]){
        cell = [self.delegate mTableView:self cellForRowAtIndexPath:indexPath];
    }return cell;
}
- (UIView *)delegate_openCellForRowAtIndexPath:(NSIndexPath *)indexPath{
    UIView *view = nil;
    if (self.delegate && [self.delegate respondsToSelector:@selector(mTableView: openCellForRowAtIndexPath:)]){
        view = [self.delegate mTableView:self openCellForRowAtIndexPath:indexPath];
    }return view;
}
