...
- (void)loadData:(NSInteger)page{
    ...
    if ([[Utility Share] offline]&&self.bk_offline) {//离线状态
    ...
    if (self.bk_data) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            // 耗时的操作
            NSArray *array = [[Utility Share] offline]?(self.bk_offline?self.bk_offline(_curPage):nil):self.bk_data(_curPage);
            dispatch_async(dispatch_get_main_queue(), ^{
                block(array,YES);
            });
        });
                 ...
                 self.task=[[AFHTTPSessionManager manager] GET:self.urlString parameters:self.postParams progress:^(NSProgress * _Nonnull downloadProgress) {
                     ...
                     self.backgroundView=nil;
                     self.strNullTitle=[responseObject valueForJSONKey:@"info"];
                     if ([[responseObject valueForJSONStrKey:@"status"] isEqualToString:@"200"]) {
                         boolNull=NO;
                         //数据成功
                         self.dataDic=[NSMutableDictionary dictionaryWithDictionary:[responseObject objectForJSONKey:@"data"]];
                         block([[responseObject objectForJSONKey:@"data"] objectForJSONKey:@"list"],NO);
                     }else{
                         boolNull=YES;
                         block([NSArray array],YES);
                         if (masktype != SVProgressHUDMaskTypeNone) {
                             [SVProgressHUD showImage:nil status:[[responseObject valueForJSONKey:@"info"] notEmptyOrNull]?[responseObject valueForJSONKey:@"info"]:alertErrorTxt];
                         }
                     }
